...
const benefits = [
  ...
  },
  {
    ...
    description: "TrustSEAL verified supplier with moral business policies and crystal pure transparency in all transactions.",
    icon: (
      <svg className="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function WhyChooseUs() {
  ...
  return (
    <section className="section-container bg-white" ref={ref}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {benefits.map((benefit, index) => (
          <motion.div
            ...
            className="card group hover:border-construction-orange/30 border-2 border-transparent"
          >
            <div className="text-construction-orange mb-4 group-hover:scale-110 transition-transform duration-300">
              {benefit.icon}
            </div>
      ...
      >
        {[
          { number: "TrustSEAL", label: "Verified" },
          { number: "2024", label: "Established" },
          { number: "100%", label: "Quality Focus" },
          { number: "24/7", label: "Customer Support" },
        ].map((stat, index) => (
          <div key={stat.label} className="text-center">
            <div className="text-4xl md:text-5xl font-bold text-construction-orange mb-2">
              {stat.number}
            </div>
